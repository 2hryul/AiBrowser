...
STOP CONDITIONS 그대로 M4b 는 시작하지 않았다. 7.6B 모델로 대체하는 것은 고르지 않았다 —
M4b 의 성공 판정은 "3회 중 2회 성공" 이고, 모델을 바꾸면 그 숫자가 무엇을 뜻하는지 알 수 없게 된다
(FIXED DECISIONS: "도구 표면을 우회하는 방식으로 성공률을 올리지 않는다" 와 같은 이유다).
